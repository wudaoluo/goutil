...
func NewFileClient(configfile string, filter string) (*Client, error) {
	return &Client{configfile: configfile, filter: filter}, nil
}
...
VarsLoop:
	...
	log.Println(fmt.Sprintf("Key Map: %#v", vars))
	return vars, nil
}
